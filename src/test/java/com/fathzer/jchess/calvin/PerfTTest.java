...
//	@Test
	void showDivide() {
		try (ContextualizedExecutor<MoveGenerator<Move>> exec =new ContextualizedExecutor<>(PhysicalCores.count())) {
			final Board board = FEN.toBoard("8/8/6b1/k3p2N/8/b1PB4/K6p/8 b - - 0 1");
			final PerfT<Move> perfT = new PerfT<>(exec);
			final PerfTResult<Move> divide = perfT.divide(2, new CalvinMoveGenerator(board));
	...
	private void doTest(ContextualizedExecutor<MoveGenerator<Move>> exec, PerfTTestData test, int depth) {
		final Board board = FEN.toBoard(test.getStartPosition()+" 0 1");
		final PerfT<Move> perfT = new PerfT<>(exec);
		if (test.getSize()>=depth) {
//			try {
				final PerfTResult<Move> divide = perfT.divide(depth, new CalvinMoveGenerator(board));
